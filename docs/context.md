...
- **Capacidades do nível MedIT:**
  ...
  - **Gestão de administradores:** cria, edita e lista administradores de unidade via `GET /auth/users/admins`, `POST /auth/users/admins`, `PUT /auth/users/admins/:id` (restritas ao `medit`). A listagem admite **paginação** (`page`, `limit`) e filtros `unitId`, `search`. Na interface: menu **Plataforma → Administradores**.
  - **Consulta somente leitura** de médicos, enfermeiros e pacientes (sem cadastro por este perfil): `GET /auth/users/platform` com `roleMiddleware(MEDIT)`, paginação e filtros `unitId`, `level`, `search`. Na interface: **Plataforma → Usuários da plataforma**, com **detalhe somente leitura** por usuário. O **administrador de unidade** continua usando as telas operacionais de médicos, enfermeiros e pacientes da sua unidade.
  - **Sem medicamentos para MedIT:** todas as rotas em `/auth/medications` respondem **403** para token `medit`; o menu e a navegação direta à rota de medicamentos são bloqueados no frontend. O cartão de unidade na lista **não** encaminha o MedIT para a tela de medicamentos.
  - **API — bloqueio de mutações operacionais:** `POST`/`PUT`/`DELETE` em médicos, enfermeiros e pacientes, bem como rotas de episódio/atendimento ligadas a paciente que impliquem operação local, retornam **403** para `medit` via `forbidUserLevelsMiddleware`; leituras admitidas onde o produto prevê (ex.: `GET /auth/patients` com `unitId` continua possível para listagem por unidade se o token tiver acesso, mas a jornada de governança do MedIT prioriza `/auth/users/platform`). Leituras de **atendimentos por médico/enfermeiro** (`GET .../attendances`) também são **negadas** para `medit`.
...
- **Middleware de autorização:** `backend/src/middlewares/roleMiddleware.ts` valida papéis permitidos nas rotas críticas; `forbidUserLevelsMiddleware` nega explicitamente perfis em mutações (ex.: `medit` em rotas de corpo clínico).
- **Exclusão de usuário:** `DELETE /auth/users/:id` é **negado** para `medit` (evita remoção genérica pela área de governança sem fluxo próprio).
- **Dados do usuário por id:** `GET /auth/users/:id` devolve o documento sem `password` nem `refreshToken`.
...
6. **Aviso de deslocamento ao paciente** — Médicos e enfermeiros possuem **`workLocationLabel`** (consultório/sala ou local de triagem). Ao concluir com sucesso **`claim-triage`** ou **`claim-consultation`**, o backend acrescenta **`patientFlowNotices`** no atendimento (tipo `triage_start` ou `consult_start`, texto do local, profissional e data). O **dashboard do paciente** exibe mensagem fixa com o local e reconsulta os atendimentos em intervalo enquanto o status for **`inTriage`** ou **`inAttendance`**, de modo que um novo claim após **release** atualize o aviso.
...
- **Vínculo profissional atômico:** `POST /auth/attendances/:id/claim-triage` (enfermeira, mesma unidade), `POST /auth/attendances/:id/complete-triage` (enfermeira dona do caso em `inTriage`), `POST /auth/attendances/:id/claim-consultation` (médico, mesma unidade), `POST /auth/attendances/:id/release-triage` e `POST /auth/attendances/:id/release-consultation` (devolução ao pool quando o profissional desiste do caso). Respostas **409** quando o estado já não permite a operação (ex.: outro profissional assumiu primeiro). **Estado atual no código** — coerente com [§5.9](#59-jornada-do-paciente-pré-atendimento-remoto-chegada-e-filas) e com a nota de **claim/release** nessa secção: o **claim** ocorre **no clique da fila** do dashboard (`claim-triage` / `claim-consultation`) **antes** da navegação ao `AttendanceDetails`; ao sair do detalhe com caso em posse (`inTriage` / `inAttendance`), a UI pode solicitar **release** (modal de confirmação; *best effort*). Implementação acompanhada historicamente em https://github.com/RafaelHDSV/MedIT/issues/30.
