...
| **`vitalSigns`**     | Subdocumento com `temperature`, `bloodPressure`, `heartRate`, `oxygenSaturation` — preenchido apenas a partir de estágios de triagem em diante (não em `ON_THE_WAY`/`WAITING_TRIAGE`). |
...
| `TCC_ACTIVE_PER_DOCTOR`         | 2                                                                     | Ativos `WAITING_ATTENDANCE` atribuídos por médico TCC no dia atual.                                   |
| `TCC_ACTIVE_PER_NURSE`          | 1                                                                     | Ativos `TRIAGE_COMPLETED` atribuídos por enfermeiro TCC no dia atual.                                 |
...
- **`FULL_FLOW`**: `ON_THE_WAY` → `WAITING_TRIAGE` → `IN_TRIAGE` → `TRIAGE_COMPLETED` → `WAITING_ATTENDANCE` → `IN_ATTENDANCE` → `ATTENDANCE_COMPLETED` (7 passos).
- **Concluído** (`buildCompletedFlow`): percorre **todos** os passos; `status` final `ATTENDANCE_COMPLETED`; intervalo entre passos baseado em `riskStepMinutes(risk)` com **±30% jitter** por degrau.
- **Ativos**: gerados **apenas para o dia atual**; status parcial dentro do fluxo (ON_THE_WAY, WAITING_TRIAGE, TRIAGE_COMPLETED, WAITING_ATTENDANCE).
- **`nurseId`**: omitido em `ON_THE_WAY` e `WAITING_TRIAGE`.
- **`doctorId`**: nos ativos gerados pelo seed, permanece omitido.
...
- **Ativos TCC atribuídos:**
  - Cada médico TCC na unidade recebe 2 atendimentos em `WAITING_ATTENDANCE` com `doctorId`.
  - Cada enfermeiro TCC na unidade recebe 1 atendimento em `TRIAGE_COMPLETED` com `nurseId`.
- **Extras variados:** ~7 atendimentos adicionais em estados mistos (ON_THE_WAY, WAITING_TRIAGE, TRIAGE_COMPLETED, WAITING_ATTENDANCE).
...
- **Status removidos do domínio:** `CANCELED` e `COMPLETED` não são utilizados pelo seed.
...
2. **Fluxo hospitalar completo:** concluídos usam `FULL_FLOW` desde `ON_THE_WAY` até `ATTENDANCE_COMPLETED` com jitter temporal. Ativos usam prefixo do mesmo fluxo.
3. **Ativos só no dia atual:** nenhum documento em status "em andamento" (não `ATTENDANCE_COMPLETED`) com `date` em dia anterior ao dia da execução do seed.
...
5. **Mínimos TCC:** pacientes ≥10 concluídos; médicos ≥8 concluídos atribuídos; enfermeiros ≥8 concluídos atribuídos.
...
8. **Sem status CANCELED/COMPLETED:** nenhum atendimento gerado com esses status.
